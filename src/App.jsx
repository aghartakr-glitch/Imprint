...
const DB = [
  ...
  {g:"그래픽디자인",pub_type:"단행본",t:"한국 디자인사 수첩",designer:"슬기와 민 (Sulki and Min)",src:"https://www.sulki-min.com/wp/a-memo-for-a-korean-design-history-kr/",img:"198_한국 디자인사 수첩",kw:["그래픽디자인","단행본","한국","디자인사","수첩","기본","면은","질문과","답변이","이어지는","2단","서울남산체","윤명조"],align_title:"-",align_body:"양끝 정렬, 좌측 정렬(일부)",align_note:"좌측 정렬",f:{w:140,h:224},m:{상:20,하:27,안:20,밖:20},c:{구성:"1단",간격:0},b:{크기:9.5,행간:17,자간:0},ty:{이름:"서울남산체 / 윤명조",분류:"혼합 (명조 / 고딕)"},pn:"상단-중앙-가로",pn_x_left:"67.3mm",pn_y_left:"6.8mm",pn_x_right:"67.3mm",pn_y_right:"6.8mm",pn_size:"9pt",pn_font:"고딕",pn_style:"흑색 / 가로 / 숫자",running:"7pt",subheading:"-",footnote:"7pt",특:"기본 면은 질문과 답변이 이어지는 2단 장문 조판으로 구성되며, 질문은 색상과 들여쓰기 변화로 구분된다. 도판 면에서는 조영제의 포스터, 로고, 표지 이미지가 본문 사이에 삽입되고, 표지 초상의 픽셀 모자이크 개념이 한국 디자인사 자료집의 성격을 압축적으로 드러낸다.",summary:"디자인 연구자 강현주가 조영제와 나눈 장시간 대담을 바탕으로 한국 그래픽 디자인의 초기 장면과 제도 형성을 추적한 연구서. 표지 초상은 조영제가 만든 기업 로고를 픽셀처럼 조합해 구성하고, 내지에서는 장문 인터뷰와 도판, 로고 자료를 차분한 2단 구조와 넓은 하단 여백 속에 병치해 구술사와 시각 아카이브를 함께 제시한다.",why_dim:"장시간 대담 텍스트와 로고 자료, 작품 도판, 주석성 정보가 함께 들어가는 디자인사 연구서로서 휴대성과 장문 독서의 안정성을 동시에 확보하기 좋은 세로형 중형 판형",why_margin:"인터뷰 본문과 질문, 도판 캡션, 페이지 번호를 분리하면서 장문 독서의 호흡을 유지하기 위해 하단 여백을 넓히고, 안쪽 여백도 충분히 확보해 문서성과 읽기 안정성을 강화",why_font:"구술 인터뷰와 디자인사 자료를 함께 다루는 책이므로 본문에는 장문 독서에 적합한 명조를, 제목과 구조 정보에는 현대적이면서도 한국적 맥락을 가진 고딕을 사용해 연구서의 공공성과 역사성을 함께 확보",why_tracking:"장문 인터뷰의 판독성을 위해 자간은 중립적으로 유지하고, 질문과 답변의 위계는 색상과 배치 변화로 해결하도록 설정",layout_type:"본문 1단 + 사진 가변 + 주석 2단"},
...
export default function App() {
  ...
  async function run() {
    ...
    try {
      ...
      // footnote/running head 전용 폰트 (mixedFnOnly, mixedRhOnly)
            ...
            if (useSideNoteFootnote) {
              ...
              if (fnNums.length > 0) {
                ...
                if (missingMarkers.length > 0) {
                  ...
                  const noteNeedsRagged = grid.noteW < 18 || vg.note === 1;
