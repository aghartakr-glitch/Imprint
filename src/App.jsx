  ...
  async function run() {
    ...
    try {
      ...
      const [profile, ranked] = await Promise.all([
        ...
        Promise.resolve((() => {
          if (h) {
            // 장르(g 필드)와 출판형태(pub_type 필드)를 엄격히 구분
            // 장르 선택 → g 필드만 필터 / 출판형태 선택 → pub_type 필드만 필터
            // 절대 섞지 않음 (전시도록 선택 시 타이포그래피 항목 혼입 방지)
            const GENRE_SET = new Set([
              '건축·공간','그래픽디자인','문학','사진','시각문화·매체',
              '아트이론·비평','인문·사회','전시·큐레이션','타이포그래피','현대미술','기타',
            ]);
            const PUB_TYPE_SET = new Set([
              '단행본','아카이브','실험출판','잡지·저널','전시도록','기관출판',
            ]);
            const isGenre  = GENRE_SET.has(h);
            const isPubType = PUB_TYPE_SET.has(h);
            ...
            const RELATED_GENRE = {
              ...
              '단행본':         ['문학','인문·사회'],
              '잡지·저널':      ['아트이론·비평','시각문화·매체'],
              '실험출판':       ['아트이론·비평','타이포그래피'],
              '전시도록':       ['현대미술','전시·큐레이션'],
              '아카이브':       ['인문·사회','아트이론·비평'],
            };

            // 1차: 선택한 장르/출판형태에 엄격히 해당하는 항목만
            const primary = DB
              .map((p, i) => ({ i, p }))
              .filter(({ p }) =>
                isGenre   ? p.g.includes(h)            // 장르 → g 필드만
              : isPubType ? (p.pub_type||'').includes(h) // 출판형태 → pub_type 필드만
              : false
              );

            let pool = primary;
            if (primary.length < 5) {
              // 모수 부족 시 연관 장르/출판형태로 확장 (동일 기준 적용)
              const related = (RELATED_GENRE[h] || []);
              const extended = DB
                .map((p, i) => ({ i, p }))
                .filter(({ p }) => related.some(r =>
                  isGenre   ? p.g.includes(r)
                : isPubType ? (p.pub_type||'').includes(r)
                : false
                ));
